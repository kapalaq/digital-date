...
export function computePhase(s, now = new Date()) {
  ...
  if (p === "lobby") {
    if (s.begin[IDA] && s.begin[IDB] && bothOnline(s)) p = "stage1";
  }
  ...
  if (p === "stage4" && s.stage4.downloaded[IDA] && s.stage4.downloaded[IDB]) p = "stage5";
  if (p === "stage5" && s.stage5?.complete?.[IDA] && s.stage5?.complete?.[IDB]) p = "done";
